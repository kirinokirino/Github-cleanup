...
constexpr char myLandmarks[] = "multi_face_landmarks";
...
absl::Status RunMPPGraph() {
  ...
  std::cout << "Get calculator graph config contents: "
            << std::endl; //calculator_graph_config_contents;
  ...
  const bool load_video = false;//!absl::GetFlag(FLAGS_input_video_path).empty();
  if (load_video) {
...
*/
  ...
  ASSIGN_OR_RETURN(mediapipe::OutputStreamPoller poller,
                   graph.AddOutputStreamPoller(myLandmarks));
  ...
  while (grab_frames) {
    ...
    //std::cout << packet.GetProtoMessageLite().SerializeAsString() << std::endl;
    ...
    MP_RETURN_IF_ERROR(gpu_helper.RunInGlContext(
        [&packet, &output_frame, &gpu_helper]() -> absl::Status {
          ...
          //const NormalizedLandmarkList& result = packet.Get<NormalizedLandmarkList>();
          return absl::OkStatus();
        }));
