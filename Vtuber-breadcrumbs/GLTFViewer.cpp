...
void setup_udp(){
 ...
 int rc = bind(sockfd, (struct sockaddr *)&servaddr, serAddrLen);
...
{
    {"1Ghost", "models/ghost/1ghostie.glb"},
    {"Ghost", "models/ghost/ghostie.gltf"},
    {"Glasses", "models/glasses/glassesMaterial.gltf"},
    {"Wrinkly Ball", "models/test/ball.glb"},
...
std::vector<float> parse_point(const std::string& i_str) {
    std::vector<float> result;
    result.reserve(3);

    size_t x = i_str.find("x");
    size_t y = i_str.find("y");
    size_t z = i_str.find("z");
    auto len = i_str.length();
    auto x_substr = i_str.substr(x+1, y);
    auto y_substr = i_str.substr(y+1, z);
    auto z_substr = i_str.substr(z+1);
    result.push_back(std::stof(x_substr));
    result.push_back(std::stof(y_substr));
    result.push_back(std::stof(z_substr));
    return result;
}

std::vector<float> average_point(std::vector<float> first, std::vector<float> second) {
        std::vector<float> result;
        result.reserve(3);
        result.push_back((first[0] + second[0]) / 2.0);
        result.push_back((first[1] + second[1]) / 2.0);
        result.push_back((first[2] + second[2]) / 2.0);
        return result;
}

std::vector<float> subtract_point(std::vector<float> first, std::vector<float> second) {

        std::vector<float> result;
        result.reserve(3);
        result.push_back(first[0] - second[0]);
        result.push_back(first[1] - second[1]);
        result.push_back(first[2] - second[2]);
        return result;
}

float4x4 matrix_lerp(float4x4 first, float4x4 second, float factor) {
        if (factor > 1.0) factor = 1.0;
        if (factor < 0.0) factor = 0.0;
        float first_factor = 1.0 - factor;//float4(1.0 - factor, 1.0 - factor, 1.0 - factor, 1.0 - factor);
        float second_factor = factor;//float4(factor, factor, factor, factor);
        return float4x4(first[0][0]*first_factor+second[0][0]*second_factor,
                                        first[0][1]*first_factor+second[0][1]*second_factor,
                                        first[0][2]*first_factor+second[0][2]*second_factor,
                                        first[0][3]*first_factor+second[0][3]*second_factor,
                                        first[1][0]*first_factor+second[1][0]*second_factor,
                                        first[1][1]*first_factor+second[1][1]*second_factor,
                                        first[1][2]*first_factor+second[1][2]*second_factor,
                                        first[1][3]*first_factor+second[1][3]*second_factor,
                                        first[2][0]*first_factor+second[2][0]*second_factor,
                                        first[2][1]*first_factor+second[2][1]*second_factor,
                                        first[2][2]*first_factor+second[2][2]*second_factor,
                                        first[2][3]*first_factor+second[2][3]*second_factor,
                                        first[3][0]*first_factor+second[3][0]*second_factor,
                                        first[3][1]*first_factor+second[3][1]*second_factor,
                                        first[3][2]*first_factor+second[3][2]*second_factor,
                                        first[3][3]*first_factor+second[3][3]*second_factor);
}

float4x4 matrix = float4x4::Translation(0,0,0);
float4x4 matrix_last = float4x4::Translation(0,0,0);
float4x4 current_matrix = float4x4::Translation(0,0,0);
float4x4 inverse = float4x4::Translation(0,0,0);
float scale = 2.0;
float rotation_scale = 15.0;
std::vector<float> nose(3, 0.0);
std::vector<float> lip_top(3, 0.0);
std::vector<float> center(3, 0.0);
...
void GLTFViewer::Update(double CurrTime, double ElapsedTime)
{
  ...
  socklen_t serAddrLen = sizeof(servaddr);
    ...
    //int readStatus =  recvfrom(sockfd, buffer, sizeof(*buffer), MSG_DONTWAIT, (struct sockaddr *) &servaddr, &serAddrLen);
    int readStatus =  recv(sockfd, buffer, MAXLINE, MSG_DONTWAIT);
        if (readStatus < 0) {
                std::cout << "No message..." << *buffer << std::endl;
          } else {

        //std::cout << readStatus << std::endl;
        std::vector<std::string> points = split(buffer, ";");
        
        lip_top = parse_point(points[0]);
        nose =  parse_point(points[1]);
        auto left = parse_point(points[2]);
        auto right = parse_point(points[3]);
        center = average_point(left, right);
        auto rotation_vector = subtract_point(center, nose);
        rotation = float4x4::RotationX(rotation_vector[2] * rotation_scale) * float4x4::RotationY(rotation_vector[0] * -rotation_scale);// * float4x4::RotationZ(rotation_vector[2] * rotation_scale);
        //rotation = float4x4::RotationY(rotation_vector[0] * -rotation_scale);// * float4x4::RotationX(rotation_vector[2] * rotation_scale); 
            float4x4 translation = float4x4::Translation(lip_top[0] * scale, lip_top[1] * scale, lip_top[2] * scale);
    ...
    inverse = current_matrix.Inverse();
    current_matrix = matrix_lerp(matrix_last, matrix, (float)((counter%cycle_length)) / (float)cycle_length);
    m_Model->Transform(inverse);
    m_Model->Transform(current_matrix);
    if (m_CameraId == 0)
    {
