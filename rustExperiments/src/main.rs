...
fn main() {
    ...
    let mut cursor = Rect::new(30, 30, 10, 10);
    ...
    let mut main_loop = || {
        for event in events.poll_iter() {
            match event {
                ...
                Event::KeyDown { keycode: Some(Keycode::Q), .. } => {
                    if !v_rect.is_empty() {
                        if v_selection < v_rect.len() - 1 {
                            v_selection += 1;
                        }
                    }
                }
                Event::KeyDown { keycode: Some(Keycode::N), .. } => {
                    v_rect.push(cursor.clone());
                }
                Event::KeyDown { keycode: Some(Keycode::K), .. } => {
                    if !v_rect.is_empty() {
                        v_rect.remove(v_selection);
                        if !v_rect.is_empty() && v_selection == v_rect.len() {
                            v_selection -= 1;
                        }
                    }
                }
        ...
        let _ = renderer.set_draw_color(colors[2]);
        let _ = renderer.fill_rect(cursor);
